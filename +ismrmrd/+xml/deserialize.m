...
%   Detailed explanation goes here
    ...
    db = javaMethod('newDocumentBuilder',dbf);


    isrc = javaObject('org.xml.sax.InputSource');
    sr = javaObject('java.io.StringReader',xmlstring);
    javaMethod('setCharacterStream',isrc,sr);

    % Parse it
    dom = javaMethod('parse',db,isrc);
...
for n = 1:numChildNodes
    ...
    if (strcmp(name, 'kspace_encoding_step_1') || strcmp(name, 'kspace_encoding_step_2'))
        if strcmp(char(theChild.getParentNode.getNodeName),'encodingLimits')
            ...
            info.(name) = str2num(theChild.getTextContent());
        end
        continue;
    end
    ...
    if isStringType(name)
        if num == 1
            info.(name) = char(theChild.getTextContent());
        else
            info.(name){num} = char(theChild.getTextContent());
        end
        continue;
    end
    ...
    if isNumericalType(name)
        if num == 1
            ...
            info.(name)(num) = str2num(theChild.getTextContent());
        end
        continue;
    end
...
function info = parseUserParameter(theNode)
    ...
    paramType = char(theNode.getNodeName());
    childNodes = theNode.getChildNodes();
    numChildNodes = childNodes.getLength();
    ...
    for n = 1:numChildNodes
        theChild = childNodes.item(n-1);
        if strcmp(theChild.getNodeName(),'name')
            info.name = char(theChild.getTextContent());
        end
        if strcmp(theChild.getNodeName(),'value')
            if strcmp(paramType, 'userParameterLong') || strcmp(paramType, 'userParameterDouble')
                info.value(end+1) = str2num(theChild.getTextContent());
            end

            if strcmp(paramType, 'userParameterString') || strcmp(paramType, 'userParameterBase64')
                info.value = char(theChild.getTextContent());
            end
        end
    end
end
